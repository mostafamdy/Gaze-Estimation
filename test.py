...
x_clf = pickle.load(open("x_clf.model", 'rb'))
y_clf = pickle.load(open("y_clf.model", 'rb'))
...
def pre_processing(data):
    data['yaw'] = np.deg2rad(data['yaw'])
    data['pitch'] = np.deg2rad(data['pitch'])
    data['roll'] = np.deg2rad(data['roll'])
    globals()['data2']=data
    for col in data2:
        if data2[col].dtypes == "object":
            xs = []
            ys = []
            xys = data2[col].values.tolist()
            for xy in xys:
                xs.append(xy[0])
                ys.append(xy[1])
            data2[col + "_x"] = xs
            data2[col + "_y"] = ys
            globals()['data2'] = data2.drop(col, axis=1)
    x_pred = x_clf.predict(data2)
    y_pred = y_clf.predict(data2)
...
class Dataset:
    def __init__(self, head, left_iris, right_iris, left_eye, right_eye):
        self.head = head
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                for iris in LEFT_IRIS:
                    left_iris.append([int(face_landmarks.landmark[iris].x * image.shape[1]),
                                      int(face_landmarks.landmark[iris].y * image.shape[0])])
                ...
                for iris in RIGHT_IRIS:
                    right_iris.append([int(face_landmarks.landmark[iris].x * image.shape[1]),
                                       int(face_landmarks.landmark[iris].y * image.shape[0])])
                ...
                # end of eye
                data=Dataset(headRot, left_ir, right_ir, left_eye, right_eye)
                dataPD=pd.DataFrame([data.to_dic()])
                # print(dataPD)
                # dataPD.to_csv("test.csv")
                (x_pred,y_pred)=pre_processing(dataPD)
                # exit()
                # break
                # if abs(x_pred-lst_x)>50 or abs(y_pred-lst_y)>50:
                lst_x=x_pred
                lst_y=y_pred
                x_pred=int(x_pred)
                y_pred=int(y_pred)
                blackWindow = np.zeros((h, w, 3), np.uint8)
                cv2.circle(blackWindow, (x_pred,y_pred), 5, [255, 0, 0], -1)
                cv2.rectangle(blackWindow,(x_pred-100,y_pred-100),(x_pred+100,y_pred+100),(255,255,255),-1)
